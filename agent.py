...
def predict_failure_risk() -> str:
    ...
    if not feature_path or not os.path.exists(feature_path):
        return "❌ Error: No feature artifacts found."
    ...
    try:
        ...
        return f"❌ Prediction model failed: {str(e)}"   
...
def perform_root_cause_analysis() -> str:
    ...
    """
    print("DEBUG: 🕵️‍♀️ RCA Agent gathering evidence...")
...
def generate_remediation_plan(previous_output: str = "") -> str:
    ...
    """
    print("DEBUG: 🛠️ Remediation Agent generating action plan...")
    ...
    print(f"DEBUG: 📝 Remediation Plan Created: {json.dumps(plan, indent=2)}")
